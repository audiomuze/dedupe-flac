...
def import_tag(tag, conn, columns):
	...
	for key, value in tag.items():
		...
		# keys[key.lower()] = key
#--------------------------------------------------------
		key = key.replace('"', '') # check for and remove '"' from tag names ... they shouldn't exist, but they've been encountered in the wild.  This shall be known henceforce as the Wietsche tax!
#--------------------------------------------------------
		keys[key.lower()] = key
